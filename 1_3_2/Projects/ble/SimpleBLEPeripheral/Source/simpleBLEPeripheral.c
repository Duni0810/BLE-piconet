...
static uint16 gapRole_ConnectionHandle = 0;
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  HalLcdWriteString( "BLE Peripheral", HAL_LCD_LINE_1 );
    ...
    // is halted
    //这条语句会让空闲的 CPU 自动进入低频以此降低功耗，注释掉代表不自动切换频率。
    // 防止串口出现乱码、丢包
    // young
    //HCI_EXT_ClkDivOnHaltCmd( HCI_EXT_ENABLE_CLK_DIVIDE_ON_HALT );
    
    //  禁止 RF 期间停止 MCU
    // 默认是 ENABLE 的， ENABLE 会让 RF 期间停止 MCU。因此需要添加本条语句，关闭它。
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SBP_PERIODIC_EVT )
  {
      ...
      // Restart timer
      if ( SBP_PERIODIC_EVT_PERIOD  && (gapProfileState == GAPROLE_CONNECTED))
      {
...
static void simpleBLEPeripheral_HandleKeys( uint8 shift, uint8 keys )
{
    ...
    if(keys & HAL_KEY_SW_1) {
        ...
        if( gapProfileState == GAPROLE_CONNECTED ) {//?D??ê?·?á??ó
            uint8 tmpLen;
...
static void simpleProfileChangeCB( uint8 paramID )
{
    ...
    case SIMPLEPROFILE_CHAR1:
        ...
        gapRole_ConnectionHandle = (uint16)(newValue);
